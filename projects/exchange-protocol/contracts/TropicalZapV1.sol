...
import {ITropicalPair} from "./interfaces/ITropicalPair.sol";
import {ITropicalRouter02} from "./interfaces/ITropicalRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

contract TropicalZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped ETH (WETH)
    IWETH public WETH;

    // TropicalRouter interface
    ITropicalRouter02 public tropicalRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from TropicalSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address TropicalRouter
    address private tropicalRouterAddress;
     ...
     * @param _tropicalRouter: address of the TropicalRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WETHAddress,
        address _tropicalRouter,
        uint256 _maxZapReverseRatio
    ) {
        WETHAddress = _WETHAddress;
        WETH = IWETH(_WETHAddress);
        tropicalRouterAddress = _tropicalRouter;
        tropicalRouter = ITropicalRouter02(_tropicalRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap ETH in a WETH pool (e.g. WETH/token)
     * @param _lpToken: LP token address (e.g. PAPPLE/ETH)
     * @param _tokenAmountOutMin: minimum token amount (e.g. PAPPLE) to receive in the intermediary swap (e.g. ETH --> PAPPLE)
     */
     ...
     * @param _lpToken: LP token address (e.g. PAPPLE/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. PAPPLE) in the intermediary swap (e.g. BUSD --> PAPPLE)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. PAPPLE/WETH)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in ETH/WETH) in the intermediary swap (e.g. PAPPLE --> ETH)
     */
    function zapOutETH(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. PAPPLE/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. PAPPLE or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. PAPPLE) in the intermediary swap (e.g. BUSD --> PAPPLE)
     */
    ...
    {
        address token0 = ITropicalPair(_lpToken).token0();
        address token1 = ITropicalPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = ITropicalPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = tropicalRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = tropicalRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == ITropicalPair(_lpToken).token0() || _token0ToZap == ITropicalPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == ITropicalPair(_lpToken).token0() || _token1ToZap == ITropicalPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == ITropicalPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / ITropicalPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / ITropicalPair(_lpToken).totalSupply();
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = ITropicalPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "TropicalRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "TropicalRouter: INSUFFICIENT_B_AMOUNT");
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = tropicalRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = tropicalRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = tropicalRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = tropicalRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = tropicalRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
